local storageUtils = require('custom.utils.storage')
...
function M.init_secrets_directory()
  ...
  if not storageUtils.ensure_directory(secrets_path) then return false end
...
function M.sync_secrets_simple()
  ...
  if not storageUtils.ensure_directory(local_path) then return false end
